...
% Pack floats to the top of float-only pages instead of spreading them apart,
% which removes the large vertical gaps between full-width table*/figure* floats.
\makeatletter
\setlength{\@fptop}{0pt}
\setlength{\@fpsep}{12pt plus 1fil}
\setlength{\@fpbot}{0pt plus 1fil}
\makeatother
% Allow more floats per page / reduce premature float pages.
...
utilisasi rendah, dua pada skenario terfragmentasi, dan satu pada skenario
penggabungan \textit{hog}, sementara \textit{HighNodeUtilization} tidak melakukan
penggusuran sama sekali pada ketiga skenario tersebut. Pada layout yang
\textit{stranding}-nya dapat dikurangi, kebijakan menurunkan skor
\textit{stranding} sebesar 73\% dan 79\%; pada layout yang sudah tak-terkurangi,
kebijakan dengan tepat menahan diri dari penggusuran yang tidak produktif. Studi
ablasi menunjukkan setiap mekanisme berperan berbeda: pengurutan RII/FSI menjaga
kualitas \textit{stranding}, \textit{guard} \textit{pack-upward} membatasi
\textit{churn}, dan pemilih berbasis \textit{bin score} menekan jumlah
penggusuran. Setiap eksekusi aman tanpa menyisakan \textit{Pod} berstatus
\textit{Pending}. Secara keseluruhan, memilih kandidat penggusuran berdasarkan
keseimbangan \textit{Node} tujuan prediksinya sudah cukup untuk mendorong
reklamasi \textit{Node} dan defragmentasi dimensional yang efektif.%
}
...
scenario, and one in the hog-merge scenario, whereas
\textit{HighNodeUtilization} performed no eviction at all in these three. Where
stranding was reducible it cut the cluster stranding score by 73\% and 79\%, and
where stranding was already irreducible it correctly declined unproductive
evictions. An ablation shows that each mechanism plays a distinct role: RII/FSI
ordering preserves stranding quality, the pack-upward guard bounds eviction
churn, and the projected-bin-score selector lowers the eviction count. Every run
was safe, leaving no Pod Pending. Overall, selecting eviction candidates by the
balance of their predicted landing Nodes is sufficient to drive effective Node
...
\input{src/paper-resourcedefrag/bab1}
\input{src/paper-resourcedefrag/bab2}
\input{src/paper-resourcedefrag/bab3}
\input{src/paper-resourcedefrag/bab4}
\input{src/paper-resourcedefrag/bab5}
