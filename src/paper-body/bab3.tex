...
\subsection{Proposed Descheduler Extension Architecture}
\label{sec:end_to_end_workflow}

This section summarizes the proposed descheduler schemes and explains how they relate to the Kubernetes Descheduler eviction workflow.

This research proposes three descheduler extensions that operate at different points of the Kubernetes Descheduler eviction decision flow. The extensions are designed as independent components rather than as a tightly coupled pipeline. Each component can be enabled separately according to the cluster operator's objective, and multiple components can be combined within the same descheduler profile when resource defragmentation, runtime-safety protection, and network-locality protection are required together.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{assets/pics/kube-descheduler-extension-position.png}
    \caption{Proposed independent descheduler extensions and their extension points within the Kubernetes Descheduler eviction workflow.}
    \label{fig:e2e_architecture_workflow}
\end{figure}

Figure~\ref{fig:e2e_architecture_workflow} summarizes the position of each proposed component in the descheduler architecture. The \textit{ResourceDefragmentation} component is implemented as a \textit{Balance} plugin. It participates in the strategy phase by analyzing the requested CPU-memory layout of the cluster, identifying fragmented or underutilized source nodes, predicting feasible landing destinations, and selecting eviction candidates that are expected to improve resource packing.

The \textit{Actual Usage Evictor} is implemented as a \textit{PreEvictionFilter}. It does not select source nodes, destination nodes, or eviction candidates. Instead, it evaluates a candidate Pod immediately before eviction by checking live CPU and memory usage. This allows the descheduler to block eviction of Pods that are currently active, even when the candidate was selected by either the proposed \textit{ResourceDefragmentation} strategy or by another descheduler strategy.

The \textit{Network-Aware Evictor} provides communication-cost protection for eviction decisions. At the global layer, the \textit{NetworkCostEvictor} acts as a \textit{PreEvictionFilter} that evaluates whether evicting a candidate Pod may lead to a placement with worse communication cost. At the strategy layer, network-aware evaluation is integrated into node-utilization strategies, namely \textit{LowNodeUtilization} and \textit{HighNodeUtilization}, so that network cost is considered earlier when candidate destinations are derived. This component can therefore be used independently with existing descheduler strategies, combined with node-utilization strategies at the strategy layer, or composed with the proposed \textit{ResourceDefragmentation} policy through the global pre-eviction filter layer.

When all proposed components are enabled together, the descheduler forms a composable eviction decision flow: a strategy proposes a candidate Pod, runtime usage filtering verifies whether the Pod is safe to evict, network-aware filtering verifies whether the eviction preserves communication locality, and only then is the eviction request submitted to the Kubernetes API. However, this combined flow is only one deployment configuration; the proposed components remain independently usable extensions.

\subsection{ResourceDefragmentation Policy Design}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{assets/pics/defrag/highlevel_defrag.png}
...
\subsubsection{Node-State Metrics}
...
\subsubsection{Drain-Candidate Detection and Ordering}
...
\subsubsection{Predicted Scheduler Target}
...
\subsubsection{Predicted-Target Eviction-Candidate Selection}
...
\subsection{Actual Usage Evictor Policy Design}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{assets/pics/actual-usage/sysoverview-diagram.png}
    \caption{Actual Usage Evictor pre-eviction filtering flow.}
    \label{fig:actualusage_sysoverview}
\end{figure}

\subsubsection{Runtime Usage Model}
...
\subsubsection{Eviction Decision Workflow and Algorithm}
\label{sec:actualusage_algorithm}

Appendix~\ref{appendix:actualusage_algorithm} summarizes the Actual Usage Evictor decision workflow. The algorithm first checks whether the candidate Pod is inside the configured namespace and label scope. Pods outside the scope are allowed immediately because the plugin is not responsible for protecting them.
...
\subsubsection{Failure and Safety Behavior}
...
\subsection{Network-Aware Evictor Policy Design}
...
\subsubsection{Network Cost Model}
...
\paragraph{Topology Base Cost Model}
...
\paragraph{Latency Base Cost Model}
...
%----------------------------------------------------------------------------- %%
\paragraph{Consistency Between Layers}
...
After passing the default pre-eviction filtering stage, our proposed Network-Aware Evictor plugin conducts an additional network-aware evaluation, as seen in Appendix~\ref{appendix:netaware_unified_algorithm}. This stage determines whether the eviction is expected to preserve or improve the efficiency of network connectivity in the cluster.
...
\noindent\begin{minipage}{\linewidth}
\lstinputlisting[
    caption={Network-Aware Layer 1: Global \textit{PreEvictionFilter} configuration},
