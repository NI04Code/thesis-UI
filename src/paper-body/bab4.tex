...
This section evaluates the developed Descheduler policies. It compares \textit{ResourceDefragmentation} with the upstream \textit{HighNodeUtilization} strategy across five node reclamation and fragmentation scenarios, evaluates how the Actual Usage Evictor protects a busy application Pod under multiple balancing strategies, and measures the latency-resource trade-off of the Network-Aware Evictor.

\subsection{ResourceDefragmentation Evaluation}
...
\subsubsection{Node Reclamation}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\linewidth]{assets/pics/defrag/chart_active_nodes.png}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\linewidth]{assets/pics/defrag/chart_avg_utilization.png}
    \caption{Average utilization of active Nodes after descheduling (\(\bar{U}\)).}
    \label{fig:defrag_avg_utilization}
\end{figure}

\subsubsection{Stranding Reduction}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\linewidth]{assets/pics/defrag/chart_stranding.png}
...
\subsubsection{S5 Heterogeneous-Drain Analysis}
...
\subsubsection{Headroom and Eviction Efficiency}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\linewidth]{assets/pics/defrag/chart_h_balanced.png}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\linewidth]{assets/pics/defrag/chart_evictions.png}
    \caption{Total Pod evictions per scenario (\(E_{count}\)).}
    \label{fig:defrag_evictions}
\end{figure}

\subsubsection{Shape-Specific Schedulability Yield}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\linewidth]{assets/pics/defrag/chart_h_cpu_skewed.png}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\linewidth]{assets/pics/defrag/chart_h_mem_skewed.png}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\linewidth]{assets/pics/defrag/chart_h_jumbo.png}
...
\subsubsection{Resource Policy Findings}
...
\subsection{Actual Usage Evictor Evaluation}
...
%-----------------------------------------------------------------------------%
\paragraph{Common Workload and Load Generation}
...
\paragraph{Experiment Execution Procedure}
\label{subsubsec:actualusage4_procedure}

Each experiment run follows a common ten-step procedure. The steps are identical across all six systems; only the Descheduler policy file and the raw capture duration differ.

\begin{enumerate}
    \item \textbf{Layout deployment.} The strategy-specific layout is deployed and validated (Section~\ref{subsubsec:actualusage4_layouts}).
    \item \textbf{Before snapshot.} Node allocatable resources, Pod placements, and cluster metrics (\(S\), \(H\), \(N_{\text{active}}\)) are recorded.
    \item \textbf{Load start.} A \textit{k6} constant-arrival-rate generator and a Pod lifecycle watcher begin recording concurrently.
    \item \textbf{Stabilization.} The system waits \(30\,s\) for the load to reach steady state.
    \item \textbf{Threshold gate.} The Kubernetes Metrics API is polled every \(10\,s\) until two consecutive samples confirm the API Pod's CPU usage-to-request ratio exceeds \(0.80\).
    \item \textbf{Event timestamp.} The current time is persisted as \(t_{\text{event}}\).
    \item \textbf{Descheduler execution.} The Descheduler runs as a single-shot Kubernetes Job with the system's policy.
    \item \textbf{Post-event capture.} The script sleeps for the configured post-event duration (\(30\,s\) for R0/R1, \(45\,s\) for H0/H1 and L0/L1) while \textit{k6} and the lifecycle watcher continue recording.
    \item \textbf{After snapshot.} The same cluster metrics are captured again.
    \item \textbf{Shutdown and summarization.} Load generation is stopped and the \textit{k6} JSON output is filtered to the \(30\,s\) event-aligned window for metric computation.
\end{enumerate}

The post-event capture for HNU and LNU uses \(45\,s\) to ensure replacement Pods reach the Ready state before the after-snapshot. During analysis, only the first \(30\,s\) of \textit{k6} data after \(t_{\text{event}}\) are retained so that failure-rate denominators are comparable across all six systems.

\paragraph{Strategy-Specific Cluster Layouts}
...
HNU uses the \textit{NodeResourcesFit} scheduler plugin with \textit{MostAllocated} scoring so replacement Pods are packed onto the three destination Nodes. LNU uses the default \textit{LeastAllocated} scheduler so evicted Pods are distributed to the underutilized Nodes. The ResourceDefragmentation pair uses the request-space layout described below.

\paragraph{ResourceDefragmentation Layout and Candidate Ordering}
...
%-----------------------------------------------------------------------------%
\paragraph{Common Application and Eviction Metrics}
...
%-----------------------------------------------------------------------------%
\paragraph{Strategy-Specific Placement Metrics}
\label{subsubsec:actualusage4_metric_strandingdelta}
%-----------------------------------------------------------------------------%
ResourceDefragmentation uses positive stranding reduction \(\Delta S\) and balanced-headroom change \(\Delta H_b\):
...
HighNodeUtilization uses active-Node reduction \(\Delta N_a\):
...
%-----------------------------------------------------------------------------%
\subsubsection{ResourceDefragmentation Results}
...
\paragraph{Eviction Substitution and Application Protection}
...
\paragraph{Defragmentation Trade-off}
...
%-----------------------------------------------------------------------------%
\subsubsection{HighNodeUtilization Results}
...
%-----------------------------------------------------------------------------%
\subsubsection{LowNodeUtilization Results}
...
%-----------------------------------------------------------------------------%
\subsubsection{Cross-Strategy Discussion}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=\linewidth]{assets/pics/actual-usage/failure_timeline_heatmap.png}
...
The three control runs exhibit compact failure intervals after the Descheduler event, whereas the corresponding Actual Usage Evictor treatments contain no failed response. Figure~\ref{fig:actualusage4_failure_bar} summarizes the failure rates across all six systems. The control configurations consistently produce failure rates between \(16.67\%\) and \(17.24\%\), while every treatment records \(0\%\).

\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.85\linewidth]{assets/pics/actual-usage/failure_rate_bar.png}
    \caption{HTTP Failure Rate Comparison Across All Strategy Pairs}
    \label{fig:actualusage4_failure_bar}
\end{figure}

Figure~\ref{fig:actualusage4_disruption_bar} shows the corresponding service disruption durations. All control runs exhibit approximately \(4.8\,s\) of disruption, while every treatment records \(0\,s\).

\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.85\linewidth]{assets/pics/actual-usage/disruption_duration_bar.png}
    \caption{Service Disruption Duration Comparison Across All Strategy Pairs}
    \label{fig:actualusage4_disruption_bar}
\end{figure}

Table~\ref{tab:actualusage4_cross_strategy} separates this common application effect from the strategy-specific placement cost. The failure-rate denominator is the number of requests completed within the common interval.
...
%-----------------------------------------------------------------------------%
\subsubsection{Actual Usage Findings}
...
\subsection{Latency and Defragmentation Trade-off Test}
...
%-----------------------------------------------------------------------------%
\subsubsection{Experimental Setup and Environment Configuration}
...
%-----------------------------------------------------------------------------%
\paragraph{Infrastructure and Cluster Topology}
...
%-----------------------------------------------------------------------------%
\paragraph{Network Profile and Topology Modeling}
...
%-----------------------------------------------------------------------------%
\paragraph{Telemetry and Monitoring Architecture}
...
%-----------------------------------------------------------------------------%
\subsubsection{Workload Profiling and Initial Distribution}
...
%-----------------------------------------------------------------------------%
\paragraph{LowNodeUtilization Scenario}
...
%-----------------------------------------------------------------------------%
\paragraph{HighNodeUtilization Scenario}
...
%-----------------------------------------------------------------------------%
\paragraph{ResourceDefragmentation Scenario}
...
To evaluate this behavior, we construct a highly fragmented cluster environment featuring massive orthogonal skews. We force the healthy nodes to become extremely CPU-heavy by packing them with the critical \textit{payment} pods and CPU-intensive network-unaware workloads. Conversely, we force the network-degraded nodes to become extremely Memory-heavy by overloading them with memory-intensive network-unaware workloads while starving them of CPU.
...
\begin{table}[tbp]
    ...
    \begin{tabular}{llcccc}
        ...
        worker-1 & 1d (Heavy Overhead) & 1 pay + 2 unaware-cpu & $\sim$1805m (90.3\%) & $\sim$416Mi (52.0\%) & \textbf{CPU SKEWED} (+0.383 RII) \\
        worker-2 & 1d (Healthy) & 1 pay + 2 unaware-cpu & $\sim$1706m (85.3\%) & $\sim$220Mi (27.5\%) & \textbf{CPU SKEWED} (+0.578 RII) \\
        worker-4 & 1a (Degraded) & 2 unaware-mem & $\sim$206m (10.3\%) & $\sim$670Mi (83.8\%) & \textbf{MEM SKEWED} (-0.735 RII) \\
        worker-5 & 1a (Degraded) & 2 unaware-mem & $\sim$206m (10.3\%) & $\sim$670Mi (83.8\%) & \textbf{MEM SKEWED} (-0.735 RII) \\
        \bottomrule
    \end{tabular}%
    }
...
%-----------------------------------------------------------------------------%
\paragraph{M2: Defragmentation Standard Deviation and Active Node Count}
...
%-----------------------------------------------------------------------------%
\paragraph{M3: Average Cross-Service Latency}
...
%-----------------------------------------------------------------------------%
\paragraph{M4: Network Degraded Pod Count}
...
%-----------------------------------------------------------------------------%
\paragraph{M5: Cluster Stranding Score (\(S\))}
...
%-----------------------------------------------------------------------------%
\subsubsection{Experimental Results: LowNodeUtilization Scenario}
...
%-----------------------------------------------------------------------------%
\paragraph{Resource Balance vs. Network Performance}
...
%-----------------------------------------------------------------------------%
\paragraph{Limitations of Pre-Eviction Filtering (L1)}
...
%-----------------------------------------------------------------------------%
\paragraph{No Network-Unaware Pods Scenario}
...
%-----------------------------------------------------------------------------%
\subsubsection{Experimental Results: HighNodeUtilization Scenario}
...
\paragraph{MinBetterCandidatePercent Threshold Tuning for Stricter Evaluation}
...
%-----------------------------------------------------------------------------%
\subsubsection{Experimental Results: ResourceDefragmentation Scenario}
...
\begin{table}[tbp]
    ...
    \begin{tabular}{l c c r c}
        ...
        \midrule
        r1: Baseline & 2 & 0.995 & 167.216 ms & 2 \\
        r2: Topology L1 & 2 & 0.995 & 165.783 ms & 2 \\
        r3: Latency L1 & 2 & 0.895 & 8.762 ms & 0 \\
        \bottomrule
    \end{tabular}%
    }
...
As shown in Table \ref{tab:resourcedefrag_results}, the baseline \textit{ResourceDefragmentation} plugin successfully identified the orthogonal CPU and Memory skews within the cluster and executed 2 evictions to aggressively balance the nodes (reducing the Stranding Score from 2.535 down to 0.995). However, because the Resource Defragmentation algorithm strictly optimizes for mathematical resource symmetry, it eagerly selected the network-sensitive \textit{payment} pods for eviction, predicting that moving them to the memory-heavy degraded nodes would perfectly resolve the cluster's resource skew. Consequently, the baseline strategy placed 2 critical pods directly into the network-degraded zone, causing a catastrophic latency spike to $\sim$167ms.
...
Because the Kube-Descheduler framework operates iteratively, blocking the \textit{payment} pods forced the Resource Defragmentation algorithm to evaluate the next-best candidates: the generic \textit{network-unaware-cpu} pods. Because these network-unaware pods were not part of the sensitive network group, our filter safely allowed their eviction. By actively steering the descheduler away from the critical workloads, we forced the cluster to resolve its resource fragmentation using network-unaware pods. This resulted in an even better reduction in resource skew (Stranding Score dropped to 0.895) while completely protecting the network integrity of the application (0 degraded pods, $\sim$8ms latency).

\paragraph{Defragmentation Performance and Network Latency Trade-offs}
...
The results from the Resource Defragmentation scenario mathematically prove that our Latency-Aware plugin does not inherently compromise the core goal of the defragmentation algorithm. Instead of blindly moving critical workloads to satisfy a mathematical equation, the plugin intelligently evaluates network safety and blocks evictions that violate it. As demonstrated by the 0.895 final Stranding Score, the presence of these network-unaware pods allowed the steered approach to achieve a slightly superior geometric resource balance compared to the baseline. However, as established in our earlier Hard Tradeoff experiments, if no such network-unaware pods were available, the Latency-Aware plugin would actively choose to leave the cluster in a fragmented state (higher Stranding Score) to guarantee network integrity like showed in \ref{tab:hard_tradeoff_results}

\subsection{End-to-End Combined Descheduler Use Case}
\label{sec:end_to_end_microservice_usecase}

This section presents an end-to-end use case that combines the three proposed mechanisms in a single Descheduler profile. The purpose is to clarify the operational position of each technique in one descheduling pass. \textit{ResourceDefragmentation} provides the request-space optimization objective at the \textit{Balance} extension point, while \textit{ActualUsageEvictor} and \textit{NetworkCostEvictor} act as pre-eviction safeguards that prevent busy or network-sensitive Pods from becoming the final eviction.

\subsubsection{Scenario Objective}

The use case is not introduced as an additional quantitative benchmark. Instead, it connects the mechanisms evaluated in the previous sections into one realistic microservice placement. The cluster contains CPU-skewed and memory-skewed Nodes, so a memory-shaped Pod from a fragmented source Node can improve request-space packing when evicted and rebound by the Kubernetes scheduler. At the same time, not every resource-useful Pod is safe to evict. A foreground API Pod may be busy at runtime, and an idle Pod may still be risky to move if it is co-located with a low-latency dependency.

The combined profile therefore represents a constrained optimization flow. \textit{ResourceDefragmentation} identifies resource-useful eviction opportunities, \textit{ActualUsageEvictor} removes candidates whose live usage is high relative to their requests, and \textit{NetworkCostEvictor} removes candidates whose possible rebinding would degrade dependency locality. The final eviction is the Pod that remains useful for defragmentation while satisfying both operational guards.

\subsubsection{Initial Cluster and Application Placement}

The scenario uses six worker Nodes grouped into two topology zones. Nodes \textit{worker-1}, \textit{worker-2}, and \textit{worker-3} are labeled as \textit{zone-a} and act as CPU-skewed expected landing targets. Nodes \textit{worker-4} and \textit{worker-5} are memory-skewed background Nodes in \textit{zone-b}. The fragmented source Node is \textit{worker-6}, also in \textit{zone-b}. The aggregate initial placement is summarized in Table~\ref{tab:e2e_cluster_layout}.

\begin{table}[tbp]
    \centering
    \caption{End-to-End Use Case: Aggregate Initial Cluster Placement}
    \label{tab:e2e_cluster_layout}
    \renewcommand{\arraystretch}{1.2}
    \scriptsize
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.19\linewidth}
        ...
        \toprule
        \textbf{Nodes} & \textbf{Zone} & \textbf{Resident Workloads} & \textbf{Placement Role} \\
        \midrule
        worker-1--worker-3 & zone-a & Each Node hosts two CPU-shaped Pods, each requesting $750\,m/25\,Mi$ & CPU-skewed expected landing targets \\
        worker-4--worker-5 & zone-b & Each Node hosts two memory-shaped Pods, each requesting $50\,m/240\,Mi$ & Memory-skewed background Nodes \\
        worker-6 & zone-b & Hosts API, chatty, cache, and safe-memory Pods with total requests $200\,m/760\,Mi$ & Fragmented source Node \\
        \bottomrule
    \end{tabular}
\end{table}

This layout creates complementary resource shapes across the cluster. The first three Nodes have high requested CPU with little requested memory, so they are suitable expected targets for a memory-shaped Pod from the source Node. The next two Nodes keep the cluster heterogeneous and prevent the example from becoming a simple empty-node drain. The source Node hosts four application Pods from the combined-plugin workload, as summarized in Table~\ref{tab:e2e_source_pods}.

\begin{table}[tbp]
    \centering
    \caption{End-to-End Use Case: Source Node Pods}
    \label{tab:e2e_source_pods}
    \renewcommand{\arraystretch}{1.2}
    \scriptsize
    \setlength{\tabcolsep}{2pt}
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.20\linewidth}
                    ...
                    >{\raggedright\arraybackslash}p{0.23\linewidth}@{}}
        \toprule
        \textbf{Pod} & \textbf{CPU Request} & \textbf{Memory Request} & \textbf{Runtime State} & \textbf{Network Role} \\
        \midrule
        workload-api & $50\,m$ & $300\,Mi$ & Busy foreground API traffic & No network group \\
        workload-chatty & $50\,m$ & $240\,Mi$ & Idle & checkout network group \\
        workload-cache & $50\,m$ & $40\,Mi$ & Idle & checkout network group \\
        workload-mem-safe & $50\,m$ & $180\,Mi$ & Idle & No network group \\
        \bottomrule
    \end{tabular}
\end{table}

This placement makes the roles of the three mechanisms visible. From a resource-only perspective, \textit{workload-api} is the strongest memory-shaped candidate because it has the largest memory request on the source Node. \textit{workload-chatty} is also useful for resource defragmentation, but it belongs to the same checkout communication group as \textit{workload-cache}. \textit{workload-mem-safe} has a smaller memory request than the first two candidates, but it is idle and has no network dependency label.

\subsubsection{End-to-End Eviction Decision Flow}

The combined Descheduler profile executes the following decision flow.

\begin{enumerate}
    \item \textbf{ResourceDefragmentation candidate targeting.}
    \textit{ResourceDefragmentation} first identifies \textit{worker-6} as a source Node because its request-space shape is memory-skewed and can be improved by relocating one memory-shaped Pod toward a CPU-skewed target. In a resource-only interpretation, \textit{workload-api} offers the highest projected target bin-score improvement because its $300\,Mi$ memory request gives the strongest shape correction. The next useful candidates are \textit{workload-chatty} and \textit{workload-mem-safe}, which still provide memory-shaped relocation opportunities.

    \item \textbf{Runtime guard protects API.}
    A resource-useful candidate is not automatically evicted. It must satisfy the configured pre-eviction filter chain before becoming the final eviction. \textit{ActualUsageEvictor} reads the latest Metrics API sample and compares each scoped Pod's CPU and memory usage with its declared requests. Since \textit{workload-api} is serving foreground API traffic and exceeds the configured $0.80$ usage-to-request threshold, the actual-usage guard prevents it from becoming the final eviction. The Descheduler therefore does not evict the busy API Pod.

    \item \textbf{NetworkCostEvictor protects checkout locality.}
    After the busy API Pod is guarded, \textit{workload-chatty} remains a useful memory-shaped candidate and passes the actual-usage guard because it is idle. However, it carries $network\mbox{-}group=checkout$. \textit{NetworkCostEvictor} discovers \textit{workload-cache} as a dependency Pod with the same group and computes the current communication cost from the candidate's current Node to the dependency placement. Because both Pods are initially co-located on \textit{worker-6}, the current communication cost is zero. The filter then evaluates possible schedulable Nodes as rebinding candidates; moving \textit{workload-chatty} away from \textit{worker-6} would increase the cost, especially for cross-zone placements. Since too few candidate Nodes preserve or improve the current cost under the configured threshold, the network-aware guard prevents this candidate from becoming the final eviction.

    \item \textbf{workload-mem-safe becomes the final eviction target.}
    The remaining safe candidate is \textit{workload-mem-safe}. It is idle, has no $network\mbox{-}group$ label, and still contributes a memory-shaped relocation opportunity for the defragmentation objective. It therefore satisfies both pre-eviction safeguards and becomes the final eviction target. The eviction returns the Pod to the Kubernetes scheduler; the Descheduler records the expected target for its own decision process, but the final binding is still performed by kube-scheduler.
\end{enumerate}

\subsubsection{Discussion}

This end-to-end scenario shows that the proposed architecture does not replace the resource objective with safety checks. Instead, it constrains the resource objective with runtime and network-locality information. \textit{ResourceDefragmentation} supplies the direction of optimization by identifying a fragmented source Node and selecting Pods whose request shape can improve the target bin score. \textit{ActualUsageEvictor} adds runtime safety by preventing eviction of Pods that are currently busy relative to their declared requests. \textit{NetworkCostEvictor} adds locality safety by rejecting Pods whose communication group would likely be rebound to a worse network placement.

The combined result is not necessarily the Pod that is most optimal under resource shape alone. In this scenario, the most resource-useful candidate is the busy API Pod, and the next candidate is network-sensitive. The final eviction is therefore \textit{workload-mem-safe}: a Pod that still helps defragmentation, but does so under operational constraints. This illustrates the intended position of each proposed technique in the Descheduler pipeline. The strategy determines where resource correction is useful, the pre-eviction filters determine which candidates remain safe to evict, and kube-scheduler performs the final placement after the eviction.
