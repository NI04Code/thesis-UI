...
The \texttt{LowNodeUtilization} strategy resolves resource packing by spreading pods from overutilized nodes to underutilized ones. To evaluate this, we aggressively pack both critical \texttt{payment} pods and heavy network-unaware workloads onto a small subset of nodes, pushing their utilization well above the target thresholds. The remainder of the cluster (including the degraded zone) is left completely empty.
...
\subsubsection{Defragmentation Standard Deviation and Active Node Count}
