...
To evaluate the effectiveness of the proposed Network-Aware Descheduler against resource imbalance and degraded latency anomalies, we built a physical-to-logical hybrid testing sandbox. The design measures the trade-off between latency preservation and compute resource consolidation by validating descheduling actions against varying node performance tiers under tight resource ceilings, providing deterministic telemetry data for evaluation.
...
To capture and aggregate this data, a minimal installation of Prometheus was deployed using kube-prometheus-stack Helm chart. Because of the strict memory limits of the \textit{t3.micro} nodes, we limited the monitoring stack to prevent system crashes. High-overhead default dependencies including Grafana, Alertmanager, kube-state-metrics, and node-exporter were entirely disabled.

The Prometheus server instance was pinned to a dedicated node (worker-6) using a targeted \emph{nodeSelector}. Its operational resource limits were tightly bounded to prevent memory leakage from exhausting the node's approximately 800 MiB allocatable ceiling.
...
The telemetry data is collected by evaluating two core histogram vectors: \texttt{peers\_response\_time\_s\_sum} and \texttt{peers\_response\_time\_s\_count}. The network-aware descheduler queries these metrics using an integrated PromQL engine over a rolling 5-minute mathematical average latency via the following unified query string, where \(G_{\text{sum}}\) and \(G_{\text{count}}\) denote those two Goldpinger vectors:
...
The evaluation is split into two experimental testing scenarios: LowNodeUtilization and HighNodeUtilization. For each scenario, we tested a combination of strategies as shown in the experimental matrix defined below:


We also applied specific plugin configurations for each scenario as follows:
...
\begin{itemize}
    ...
    \item \textbf{High Node Utilization:} This scenario identifies nodes with less than 30\% CPU and 40\% Memory resource utilization as underutilized nodes, and nodes with greater usage in either metric as overutilized nodes to support node downscaling.
\end{itemize}
...
By strategically deploying network-unaware pods alongside our critical \texttt{payment} services, we force the cluster into specific initial imbalance states tailored for each scenario.
...
\begin{table*}[tbp]
    ...
    \begin{tabular}{@{}lcc@{}}
            ...
            \midrule
        \end{tabular}
    \caption{Experimental Matrix Mapping for Latency and Imbalance Trade-off Test}
...
The \texttt{LowNodeUtilization} strategy addresses resource imbalance by spreading pods from overutilized nodes to underutilized ones. To evaluate this, we aggressively pack both critical \texttt{payment} pods and heavy network-unaware workloads onto a small subset of nodes, pushing their utilization well above the target thresholds. The remainder of the cluster (including the degraded zone) is left completely empty.

Table \ref{tab:initial_state_merged} details the exact state of the cluster prior to descheduler intervention for both evaluation strategies.

\begin{table*}[tbp]
    \centering
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{l l c l c c l}
        \toprule
        \textbf{Strategy} & \textbf{Node} & \textbf{Zone} & \textbf{Pods} & \textbf{CPU Alloc.} & \textbf{Mem Alloc.} & \textbf{Status} \\
        \midrule
        \multirow{5}{*}{\textbf{LowNodeUtilization}} & worker-1 & 1d (Healthy) & 1 pay + 5 unaware & $\sim$1000m (55\%) & $\sim$304Mi (38\%) & Overutilized ($>$50\%) \\
        & worker-2 & 1d (Healthy) & 1 pay + 5 unaware & $\sim$1000m (55\%) & $\sim$304Mi (38\%) & Overutilized ($>$50\%) \\
        & worker-3 & 1b (Healthy) & 1 db + 2 trx & $\sim$650m (36\%) & $\sim$246Mi (30\%) & Normal (in bounds) \\
        & worker-4 & 1a (Degraded) & 0 pods & 0m (0\%) & 0Mi (0\%) & Underutilized ($<$20\%) \\
        & worker-5 & 1a (Degraded) & 0 pods & 0m (0\%) & 0Mi (0\%) & Underutilized ($<$20\%) \\
        \midrule
        \multirow{5}{*}{\textbf{HighNodeUtilization}} & worker-1 & 1d (Healthy)  & 1 pay + 1 unaware & $\sim$306m (15.3\%) & $\sim$152Mi (19.0\%) & Underutilized ($<$30\%) \\
        & worker-2 & 1d (Healthy)  & 1 pay + 1 unaware & $\sim$306m (15.3\%) & $\sim$152Mi (19.0\%) & Underutilized ($<$30\%) \\
        & worker-3 & 1b (Healthy)  & 1 db + 2 trx & $\sim$756m (37.8\%) & $\sim$366Mi (45.7\%) & Overutilized ($>$30\%) \\
        & worker-4 & 1a (Degraded) & 6 unaware & $\sim$1006m (50.3\%) & $\sim$408Mi (51.0\%) & Overutilized ($>$30\%) \\
        & worker-5 & 1a (Degraded) & 5 unaware & $\sim$856m (42.8\%) & $\sim$360Mi (45.0\%) & Overutilized ($>$30\%) \\
        \bottomrule
    \end{tabular}%
    }
    \caption{Resource Allocation: Initial State for Evaluated Strategies}
    \label{tab:initial_state_merged}
\end{table*}
...
Conversely, the \texttt{HighNodeUtilization} strategy is designed to pack a fragmented cluster by evicting pods from underutilized nodes and packing them onto overutilized nodes. We place the critical \texttt{payment} pods sparsely across otherwise empty nodes. We then intentionally overload the remaining nodes exclusively with heavy network-unaware workloads, creating the heaviest "gravity wells" within the network-degraded zone.

Table \ref{tab:initial_state_merged} details the exact state of the cluster prior to node reclamation for this scenario.

This starting state forces the descheduler to evict the \texttt{payment} pods to pack the cluster. The challenge is preventing the \texttt{kube-scheduler}'s default \texttt{MostAllocated} strategy from blindly bin-packing these critical workloads into the overloaded, but network-degraded, nodes.
...
\label{subsec:netaware4_metrics}

\subsubsection{Eviction Count}
...
From an architectural standpoint, network-aware configurations are structurally expected to show a decreased eviction profile compared to the unrestricted baseline. This contraction representatively isolates the "cost of network-awareness," where the integrated cost filters purposely block rebalancing movements that would otherwise degrade inter-service network latency bounds. This metric is evaluated universally across both testing scenarios (\texttt{LowNodeUtilization} and \texttt{HighNodeUtilization}) to quantify the baseline operational cost and activity level of each filter.


\subsubsection{Imbalance Standard Deviation and Active Node Count}
\label{subsubsec:netaware4_evaluationmetrics2}

To properly evaluate and distinguish between the opposing structural goals of load imbalance and bin-packing, we create a standard framework to measure cluster resource standard deviation. The evaluation is performed independently across both CPU and memory resource dimensions, where the utilization coefficient \(U_{n,R}\) for each worker node \(n\) and resource dimension \(R \in \{\text{cpu}, \text{memory}\}\) is defined as follows:
...
By combining \(\sigma_R\) and \(N_{\text{active}}\), the framework evaluates whether the descheduling policy achieves its intended infrastructure objective. A low \(\sigma_R\) together with a high \(N_{\text{active}}\) indicates successful imbalance, where workloads are distributed evenly across the cluster infrastructure. Conversely, a high \(\sigma_R\) combined with a low \(N_{\text{active}}\) indicates successful bin-packing, where workloads are packed onto a smaller subset of nodes while leaving other nodes idle and available for scale-down. An inefficient cluster state is characterized by both high \(\sigma_R\) and high \(N_{\text{active}}\), indicating that workloads remain unevenly distributed while still occupying most of the infrastructure footprint. The \(\sigma_R\) metric is primarily utilized in the \texttt{LowNodeUtilization} and \texttt{HighNodeUtilization} scenarios to evaluate the absolute spread of cluster resources. Conversely, the Active Node Count (\(N_{\text{active}}\)) is specifically leveraged in the \texttt{HighNodeUtilization} scenario to measure bin-packing efficiency and node-reclamation capability.


\subsubsection{Average Cross-Service Latency}
...
\begin{itemize}
    ...
    \item \(p_1\) and \(p_2\) are running pods belonging to different services (\(S_x\) and \(S_y\)) within the \texttt{checkout-flow} application group.
...
This is a universal metric applied across both scenarios (\texttt{LowNodeUtilization} and \texttt{HighNodeUtilization}) as the primary and most critical indicator of network performance preservation.


\subsubsection{Network Degraded Pod Count}
...
\begin{itemize}
    ...
    \item \(G_{\text{checkout}}\) represents the set of active pods with network-group label \texttt{checkout-flow} identifier.
...
\texttt{HighNodeUtilization} scenario presents a unique challenge for the descheduler. Unlike \texttt{LowNodeUtilization}, which spreads pods across the cluster, \texttt{HighNodeUtilization} seeks to pack workloads by evicting pods from underutilized nodes and packing them onto overutilized ones. This relies heavily on the \texttt{kube-scheduler}'s \texttt{MostAllocated} scoring strategy, which creates a centered gravity towards heavily loaded nodes when scheduling pods.
...
Because the descheduler cannot guarantee final placement, it must assess the statistical risk of the \texttt{kube-scheduler}'s behavior. We best demonstrate the necessity of the strict 75\% threshold by analyzing the catastrophic failure of the Latency-Aware L2 filter when configured with a looser 50\% threshold.

As shown in Table \ref{tab:highnode_results}, both the L1 and L2 latency filters failed when configured with a loose 50\% threshold. When evaluating the L1 Pre-Eviction filter, it scanned 4 potential candidate nodes and found that 2 of them (worker-2 and worker-3) offered equal or better latency. Because this mathematically satisfied the 50\% threshold, L1 authorized the evictions. However, the \texttt{kube-scheduler} subsequently placed one of the sensitive pods into the degraded zone, causing average latency to spike to $\sim$63ms.

Same case happen to latency L2 filter at 50\% that had $\sim$122ms latency spike matching the spike in baseline and topology filter. This occurred due to an architectural boundary between the descheduler and the \texttt{kube-scheduler}. The L2 filter strictly evaluated the three highly-utilized target nodes and found that one of them (worker-3) was healthy (0ms latency). Because 1 out of 3 targets satisfied the loose 50\% minimum threshold, the filter approved the eviction. However, once the pod was returned to the scheduling queue, the \texttt{kube-scheduler}'s \texttt{MostAllocated} algorithm bypassed the healthy node and packed the pods onto the degraded nodes because they had a higher usage allocation.
...
\begin{figure}[tbp]
    ...
    \caption{Performance comparison of \texttt{MinBetterCandidatePercent} Tuning}
