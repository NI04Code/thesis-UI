...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{assets/pics/e2e_workflow.png}
...
\begin{enumerate}
    ...
    \item \textbf{First Safety Gate (Actual Utilization):} Once a candidate pod is proposed, the \textit{ActualUsage Evictor} acts as the first \textit{PreEvictionFilter}. It checks the pod's live CPU and memory metrics. If the pod is actively serving traffic (usage exceeds a threshold relative to its requests), the eviction is blocked to avoid service disruption, forcing the strategy to propose the next best candidate.
    \item \textbf{Second Safety Gate (Network Latency):} If the pod is not actively busy, the \textit{NetworkCost Evictor} acts as the final \textit{PreEvictionFilter}. It evaluates the predicted destination node against the pod's network dependencies. It computes whether relocating the pod would result in equal or better communication costs compared to its current placement. If the eviction is likely to degrade overall network efficiency, meaning there are too few candidate nodes that can offer a comparable or improved network cost, the eviction is blocked.
\end{enumerate}
