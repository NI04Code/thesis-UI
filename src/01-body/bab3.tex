...
For a Pod \(p\), let \(R_{p,c}\) and \(R_{p,m}\) denote the total requested CPU and memory across all containers. Let \(A_{p,c}\) and \(A_{p,m}\) denote the actual CPU and memory usage obtained from Pod metrics. The runtime usage ratios are defined as:
...
where \(\rho_{p,c}\) is the actual CPU usage ratio and \(\rho_{p,m}\) is the actual memory usage ratio. CPU is aggregated in millicores, while memory is aggregated in bytes. The ratios compare current resource usage against the Pod's declared requests rather than against node capacity, because the filter evaluates whether the Pod itself is currently active relative to the resources reserved for it.
...
The default thresholds are \(\theta_c = 0.80\) for CPU and \(\theta_m = 0.80\) for memory. Threshold values greater than one are also valid, allowing deployments to tolerate temporary usage above the declared request before a Pod is considered too busy to evict.

The implementation treats zero-request cases conservatively. If a resource request is zero and the corresponding actual usage is positive, the usage ratio is treated as infinite and the eviction is blocked. If both request and usage are zero, the ratio is treated as zero and does not block eviction.
...
For scoped Pods, the plugin retrieves the latest Pod metrics from the metrics client. It then aggregates declared requests and observed usage across all containers. The resulting CPU and memory usage ratios are compared with the configured thresholds. If either ratio is greater than or equal to its threshold, the Pod is considered busy and the eviction is rejected. Otherwise, the Pod passes the filter and the descheduler may continue with the next pre-eviction filter or the final eviction request.
...
The Actual Usage Evictor uses a fail-closed behavior for Pods inside its scope. If the metrics collector or metrics client is unavailable, the eviction is blocked. If the Pod metrics cannot be read, the eviction is also blocked. The same behavior applies when the metrics are incomplete, for example when a container in the Pod specification has no corresponding CPU or memory metric.

This conservative behavior is intentional because the plugin's role is to protect active Pods. When runtime usage cannot be verified, allowing the eviction could move a busy Pod without evidence that it is safe to do so. By blocking the eviction on missing or incomplete metrics, the plugin avoids making eviction decisions from stale or unreliable runtime information.
...
\begin{table}[H]
    ...
    \begin{tabular}{l l l p{7.0cm}}
        ...
        \hline
        \textit{cpuUsageThreshold} & float & \(0.80\) & Blocks eviction when actual CPU usage divided by requested CPU reaches this value. \\
        \textit{memoryUsageThreshold} & float & \(0.80\) & Blocks eviction when actual memory usage divided by requested memory reaches this value. \\
