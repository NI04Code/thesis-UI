...
where \(B_n\) is the bin score of Node \(n\), combining density (average utilization) with dimensional balance (the penalty factor \(1-|u_{n,c}-u_{n,m}|\) reduces the score when CPU and memory utilization diverge). A dense and balanced Node has a high \(B_n\), whereas an empty or strongly skewed Node has a lower score. The signed Resource Imbalance Index identifies the skew direction:

\[
    RII_n = u_{n,c} - u_{n,m},
\]

where \(RII_n\) is the Resource Imbalance Index of Node \(n\). A positive value indicates CPU-heavy skew (CPU utilization exceeds memory utilization), while a negative value indicates memory-heavy skew. The Free-Space Index represents joint CPU-memory headroom:
...
where \(u_{t,c}\) and \(u_{t,m}\) are the projected CPU and memory utilization of target Node \(t\) after receiving Pod \(p\), while \(u_{s,c}\) and \(u_{s,m}\) are the current CPU and memory utilization of source Node \(s\). The constraint requires that the minimum utilization dimension of the target (its less-utilized resource) must be at least as high as the minimum utilization dimension of the source. It prevents an eviction from merely moving a Pod to an emptier Node. Among feasible Nodes, \textit{predictSchedulerTarget} returns the Node with the highest projected bin score after receiving \(p\).
