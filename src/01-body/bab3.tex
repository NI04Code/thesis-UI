...
\begin{equation}
    ...
    \quad \lor \quad
    \rho_{p,m} \geq \theta_m.
    \label{eq:actualusage_block_condition}
\end{equation}
...
\begin{equation}
    ...
    \quad \land \quad
    \rho_{p,m} < \theta_m.
    \label{eq:actualusage_allow_condition}
\end{equation}
...
With the default \texttt{Allow} policy, a missing dimension is skipped and contributes \(\mathrm{busy}=\mathrm{false}\). The other resource dimension can still block eviction through the OR condition in Equation~\ref{eq:actualusage_block_condition}. With \texttt{Block}, a missing dimension contributes \(\mathrm{busy}=\mathrm{true}\) and blocks eviction. The default does not assert that a Pod without requests is idle or that eviction is harmless. It indicates only that the plugin has no request-relative denominator from which to infer busyness. Operators can select \texttt{Block} when a conservative missing-request behavior is required.
...
Missing requests follow \textit{missingRequestPolicy} instead of the metrics failure path. This distinction is necessary because unavailable metrics leave the plugin unable to observe whether the Pod is busy, whereas a missing request leaves the plugin without a denominator for interpreting an otherwise available observation. The default \texttt{Allow} policy skips only the affected resource dimension; it does not change the fail-closed behavior for missing or incomplete metrics.
