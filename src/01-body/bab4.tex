...
\begin{itemize}
    ...
    \item \textbf{ResourceDefragmentation (RD):} the proposed policy using request-based Node state, predicted feasible targets, and projected target bin score as its victim criterion.
\end{itemize}
...
S1 primarily tests Node reclamation. S2 and S4 test whether dimensional fragmentation can be reduced when a scalar utilization baseline has no suitable action. S3 combines node reclamation and shape preservation. S5 isolates shape-aware victim selection during a heterogeneous drain.
