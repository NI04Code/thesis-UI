...
\begin{enumerate}
    \item \textbf{Checkout Flow Pods:} A tightly coupled microservice chain consisting of 5 replicas across three application architectures:
    ...
    Every pod within these deployments is stamped with an explicit metadata label: \textit{network-group: checkout-flow}. This tag informs our descheduler that these components are latency-sensitive and share cross-service dependencies.
...
\begin{table}[H]
    ...
    \vspace{0.5em} 
    \caption{Workload profiling and resource allocation}
