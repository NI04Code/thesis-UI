...
%-----------------------------------------------------------------------------%
A single \textit{k6} process drives constant-arrival-rate traffic to the api Service throughout the run. The request rate is calibrated so that the api Pod's actual CPU usage exceeds the configured eviction threshold ratio of \(0.80\) under sustained load while remaining safely below its CPU limit. Each request consumes a fixed amount of CPU work in the application container, which produces a stable usage signal at the Metrics API.
