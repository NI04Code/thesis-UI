...
\section{End-to-End Realistic Microservice Use Case}
\label{sec:end_to_end_microservice_usecase}

To illustrate the complete workflow of the proposed descheduler architecture, this section presents a realistic end-to-end scenario combining resource defragmentation, actual-usage gating, and network-aware gating. 

\subsection{Cluster Topology and Application Workload}
Consider a multi-zone Kubernetes cluster with worker nodes distributed across \textit{us-east-1a}, \textit{us-east-1b}, and a simulated distant zone \textit{eu-west-1a} (with an artificial $\sim$80ms network latency). The cluster hosts an e-commerce microservice application comprising several components:
\begin{itemize}
    \item \textbf{Payment Service}: A critical, user-facing frontend component.
    \item \textbf{Transaction Service}: A backend service tightly coupled with the Payment service, heavily dependent on low-latency communication.
    \item \textbf{Network-Unaware Worker}: A non-latency-sensitive batch processing pod.
\end{itemize}

Over time, scaling events have caused the cluster to become structurally fragmented. Compute capacity is available but scattered in incompatible CPU-heavy and memory-heavy shapes across multiple underutilized nodes. 

\subsection{End-to-End Descheduler Operation}
The end-to-end pipeline evaluates the cluster state and executes the eviction process through three distinct stages:

\begin{enumerate}
    \item \textbf{Request-Based Strategy Phase:} 
    The \textit{ResourceDefragmentation} plugin analyzes the cluster's fragmented layout. It identifies a highly underutilized source node hosting the Payment, Transaction, and Network-Unaware Worker pods. Evaluating request-space bin packing, the strategy determines that evicting the \textbf{Payment pod} to a specific destination node will yield the highest projected bin score and significantly reduce the cluster's stranding score.
    
    \item \textbf{Actual Utilization Gating:} 
    Before executing the eviction, the \textit{Actual Usage Evictor} intercepts the request. It queries the live metrics API and observes that the Payment pod is currently processing a spike in checkout traffic, with its CPU usage exceeding the 80\% threshold relative to its requested baseline. To prevent immediate service disruption to users, the filter \textbf{blocks} the eviction. The strategy falls back to its second-best candidate: the \textbf{Transaction pod}.
    
    \item \textbf{Network Latency Gating:} 
    The Transaction pod passes the Actual Usage check because it is currently idle. However, the \textit{NetworkCost Evictor} intercepts the eviction. It evaluates the predicted destination node for the Transaction pod, which happens to be located in the distant \textit{eu-west-1a} zone. Moving the pod there would introduce $\sim$80ms of latency between the Transaction service and its dependent Payment service. Recognizing the severe performance penalty, the network filter \textbf{blocks} the eviction. 
\end{enumerate}

Finally, the strategy proposes the third candidate: the \textbf{Network-Unaware Worker pod}. The Network-Unaware Worker is neither actively busy nor bound by tight network dependencies. It safely passes both the Actual Usage and Network Cost pre-eviction filters. The descheduler executes the eviction, allowing the cluster to consolidate resources without disrupting active traffic or degrading microservice communication.

\subsection{Scheduler Configuration Adjustments}
\label{subsec:scheduler_configuration_adjustments}

For this end-to-end architecture to perform optimally, the descheduler's intent must be aligned with the Kubernetes scheduler. Because the descheduler operates exclusively as a post-placement correction tool, it cannot force the final pod placement; it relies on the kube-scheduler to select the predicted target node.

In this study, the kube-scheduler relies strictly on default scoring plugins (such as \textit{LeastAllocated} for spreading scenarios, or \textit{MostAllocated} paired with \textit{BalancedAllocation} for the \textit{ResourceDefragmentation} strategy) to drive pod placement. When the descheduler evicts a pod to improve packing density, a packing-oriented scheduler profile naturally favors nodes with the most allocated resources, indirectly aligning with the descheduler's defragmentation goals. 

However, because we do not employ custom scheduler setups (such as tightly tuned \textit{PodTopologySpread} constraints or custom \textit{InterPodAffinity} weights), there is a risk that the kube-scheduler may inadvertently place an evicted pod onto a node that degrades network latency if it blindly follows raw compute availability. Therefore, to fully guarantee that the descheduler's network-aware intent is respected during the rebinding phase, administrators must adjust the kube-scheduler configuration. Specifically, introducing stronger \textit{NodeAffinity} rules or deploying a custom scheduler profile that incorporates network topology weights is necessary to achieve perfect synchronization between the descheduler's eviction logic and the scheduler's placement logic.
