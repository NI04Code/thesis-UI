...
Application results are measured during the \(30\) seconds after the recorded Descheduler event. Failure-rate denominators contain only requests completed within this event-aligned window.
...
\begin{enumerate}
    ...
    \item \textbf{Post-event capture.} The script records \textit{k6} output and Pod lifecycle events for \(30\,s\) after the event.
    \item \textbf{After snapshot.} The same cluster metrics are captured again.
    \item \textbf{Shutdown and summarization.} Load generation is stopped and the \textit{k6} JSON output from the \(30\,s\) event-aligned window is used for metric computation.
\end{enumerate}

The same \(30\,s\) post-event observation window is used across all six systems so that failure-rate denominators are directly comparable.
