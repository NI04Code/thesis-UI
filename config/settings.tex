%-----------------------------------------------------------------------------%
% Judul Dokumen
%-----------------------------------------------------------------------------%
%
% Judul laporan.
\def\judul{Implementasi dan Evaluasi Multi-Dimensional Defragmentation dan Network-Aware Filtering Policies pada Kubernetes Descheduler}
%
% Tulis kembali judul laporan namun dengan bahasa Ingris
\def\judulInggris{Implementation and Evaluation of Multi-Dimensional Defragmentation and Network-Aware Filtering Policies for Kubernetes Descheduler}


%-----------------------------------------------------------------------------%
% Tipe Dokumen
%-----------------------------------------------------------------------------%
%
% Tipe laporan, dapat berisi: Laporan Kerja Praktik, Kampus Merdeka, Skripsi, Tugas Akhir, Thesis, atau Disertasi
\def\type{Skripsi}
%
% Nama jalur Kampus Merdeka (hanya perlu diisi jika tipe laporan adalah Kampus Merdeka
% Contoh isian (khusus Fasilkom): Studi Independen, Magang Mitra, Magang BUMN, Bangkit, Apple Academy, BYOC
\def\kampusMerdekaType{}
% Jika ada perwakilan mitra, isi dengan jabatan perwakilan mitra tersebut
% (misal: Cohort Manager)
% Kosongkan jika tidak ada perwakilan mitra
\def\partnerPosition{}
% Jika ada perwakilan mitra, isi dengan nama perusahaan atau nama program
% (misal: PT. Astra International, Bangkit Academy 2023)
% Kosongkan jika tidak ada perwakilan mitra
\def\partnerInstance{}
%
% Jenjang studi, dapat berisi: Diploma, Sarjana, Magister, atau Doktor
\def\jenjang{Sarjana}


%-----------------------------------------------------------------------------%
% Informasi Penulis
%-----------------------------------------------------------------------------%
%
% Tulis nama Anda
% Kosongkan penulisDua dan penulisTiga jika Anda melaksanakan tugas akhir/laporan secara individu
\def\penulisSatu{Naufal Ichsan} % nama lengkap penulis pertama
\def\penulisDua{Muhammad Raihan Akbar} % nama lengkap penulis kedua
\def\penulisTiga{Matthew Hotmaraja Johan Turnip} % nama lengkap penulis ketiga
%
% Tulis NPM Anda
% Kosongkan npmDua dan npmTiga jika Anda melaksanakan tugas akhir/laporan secara individu
\def\npmSatu{2206082013} % NPM penulis pertama
\def\npmDua{2206827674} % NPM penulis kedua
\def\npmTiga{2206081231} % NPM penulis ketiga
%
% Tulis Program Studi yang Anda ambil
% Kosongkan programDua dan programTiga jika Anda melaksanakan tugas akhir/laporan secara individu
\def\programSatu{Ilmu Komputer} % program studi penulis pertama
\def\programDua{Ilmu Komputer} % program studi penulis kedua
\def\programTiga{Ilmu Komputer} % program studi penulis ketiga
%
% Tulis Program Studi yang Anda ambil dalam bahasa inggris
% Kosongkan programDua dan programTiga jika Anda melaksanakan tugas akhir/laporan secara individu
\def\studyProgramSatu{Computer Science} % 1st author's study program
\def\studyProgramDua{Computer Science} % 2nd author's study program
\def\studyProgramTiga{Computer Science} % 3rd author's study program

%-----------------------------------------------------------------------------%
% Tanda Tangan Penulis (untuk Halaman Pernyataan Orisinalitas & Persetujuan Publikasi)
%-----------------------------------------------------------------------------%
% Isi dengan path gambar tanda tangan Anda (misal: assets/signature/naufal-sign.png)
% Kosongkan jika belum ada tanda tangan (seperti \def\signDua{})
\def\signSatu{assets/signature/naufal-sign.png}
\def\signDua{assets/signature/ian-sign.png}
\def\signTiga{assets/signature/matthew-sign.png}



%-----------------------------------------------------------------------------%
% Informasi Dosen Pembimbing & Penguji
%-----------------------------------------------------------------------------%
%
% Tuliskan pembimbing
% Untuk Kampus Merdeka: Tuliskan dosen PIC/pembimbing dari Fakultas Anda
\def\pembimbingSatu{Made Harta Dwijaksara, S.T., M.Sc., Ph.D.}
% S1 s.d. S3: Kosongkan jika tidak ada pembimbing kedua
% Untuk Kampus Merdeka: Tuliskan penanggung jawab/penyelia/mitra
%                       dari program Kampus Merdeka yang Anda ambil (jika ada)
\def\pembimbingDua{}
% S2 & S3: Kosongkan jika tidak ada pembimbing ketiga
\def\pembimbingTiga{}

%
% Tuliskan penguji
\def\pengujiSatu{Amril Syalim, S.Kom., M.Eng., Ph.D.}
\def\pengujiDua{Hendrie Gunawan, S.Kom., M.S.M.}
% Kosongkan jika tidak ada penguji ketiga (umumnya penguji ketiga hanya ada untuk S2)
\def\pengujiTiga{}
% Kosongkan jika tidak ada penguji keempat, kelima, atau keenam (umumnya penguji > 3 hanya ada untuk S3)
\def\pengujiEmpat{}
\def\pengujiLima{}
\def\pengujiEnam{}


%-----------------------------------------------------------------------------%
% Informasi Lain (Asal Fakultas, Tanggal, dsb.)
%-----------------------------------------------------------------------------%
%
% Tuliskan Fakultas dimana penulis berada
\def\fakultas{Ilmu Komputer}
\def\fakultasInggris{Computer Science}
%
% Tuliskan bulan dan tahun publikasi laporan
\Var{\bulanTahun}{Juni 2026}
%
% Tuliskan gelar yang akan diperoleh dengan menyerahkan laporan ini
\def\gelar{Sarjana Ilmu Komputer}
%
% Tuliskan tanggal pengesahan laporan, waktu dimana laporan diserahkan ke
% penguji/sekretariat
\def\tanggalSiapSidang{10 Juni 2026}
%
% Tuliskan tanggal keputusan sidang dikeluarkan dan penulis dinyatakan
% lulus/tidak lulus
\def\tanggalLulus{15 Juni 2026}

%-----------------------------------------------------------------------------%
% Pernyataan Penggunaan Kecerdasan Artifisial
%-----------------------------------------------------------------------------%
%
% Tuliskan platform AI yang digunakan dan tujuan penggunaannya.
% Contoh:
% \def\platformAI{ChatGPT dan GitHub Copilot}
% \def\tujuanPenggunaanAI{pemeriksaan tata bahasa dan asistensi pemrograman}
\def\platformAI{ChatGPT, Claude dan Gemini}
\def\tujuanPenggunaanAI{pemeriksaan dan perbaikan tata bahasa, dan asistensi pemrograman}
%
% Tanggal penandatanganan pernyataan penggunaan AI.
\def\tanggalPernyataanAI{06 Juni 2026}


%-----------------------------------------------------------------------------%
% Judul Setiap Bab
%-----------------------------------------------------------------------------%
%
% Berikut ada judul-judul setiap bab.
% Silahkan diubah sesuai dengan kebutuhan.
%
\Var{\kataPengantar}{Acknowledgments}
\Var{\babSatu}{Introduction}
\Var{\babDua}{Literature Review}
\Var{\babTiga}{Policies Design and Implementation}
\Var{\babEmpat}{Experiment and Evaluation}
\Var{\babLima}{Conclusion and Future Work}



%-----------------------------------------------------------------------------%
% Capitalized Variables
% Anda tidak perlu mengubah apapun di bagian ini
%-----------------------------------------------------------------------------%
\Var{\Judul}{\judul}
\Var{\Type}{\type}
\Var{\PenulisSatu}{\penulisSatu}
\Var{\PenulisDua}{\penulisDua}
\Var{\PenulisTiga}{\penulisTiga}
\Var{\Fakultas}{\fakultas}
\Var{\ProgramSatu}{\programSatu}
\Var{\ProgramDua}{\programDua}
\Var{\ProgramTiga}{\programTiga}


%-----------------------------------------------------------------------------%
% Custom settings
%-----------------------------------------------------------------------------%
\newcommand{\func}[2]{\text{#1}\left(#2\right)}
