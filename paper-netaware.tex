...
\renewcommand{\judul}{Implementation and Evaluation of a Network-Aware Filtering Policy for Kubernetes Descheduler}
...
Klaster Kubernetes yang dinamis mengakumulasi fragmentasi sumber daya, yang sering memicu kebijakan konsolidasi descheduler. Namun, konsolidasi yang murni berbasis komputasi dapat memindahkan layanan mikro yang saling bergantung melalui jaringan terdegradasi. Penelitian ini menyajikan arsitektur Kubernetes Descheduler yang menggabungkan pengamanan penggusuran berbasis kondisi jaringan. Kebijakan ini mengevaluasi bentuk penggunaan dan memilih penggusuran, sementara filter sadar latensi mencegat tindakan yang merusak jaringan. Pada evaluasi sadar jaringan, perilaku descheduler standar menyeimbangkan sumber daya komputasi tetapi menggusur beban kerja sensitif, sehingga menyebabkan lonjakan latensi hingga $\sim$167ms. Sebaliknya, filter sadar latensi mencegat tindakan yang merusak jaringan tersebut, mempertahankan latensi dasar pada $\sim$8ms, dan tetap mengarahkan penggusuran ke \textit{Pod} non-kritis. Secara keseluruhan, arsitektur yang diusulkan meningkatkan utilisasi sumber daya sekaligus melindungi beban kerja sensitif terhadap jaringan.%
}

\newcommand{\abstractEnglish}{%
Dynamic Kubernetes clusters accumulate resource fragmentation that triggers descheduler consolidation policies. However, compute-focused consolidation may move dependent microservices across degraded network links. This study presents a Kubernetes Descheduler architecture featuring a network-aware eviction safeguard. The policy selects evictions based on feasible predicted scheduler targets, while a network-aware filter blocks movements toward latency-degrading destinations. In the network-aware evaluation, the baseline descheduler behavior balanced compute resources but evicted sensitive workloads, causing latency spikes of up to $\sim$167ms. In contrast, the latency-aware filter intercepted these network-degrading actions, maintained baseline latency at $\sim$8ms, and still achieved balance by steering eviction toward non-critical Pods. Overall, the proposed architecture improves resource utilization while protecting network-sensitive workloads.%
}
...
\input{src/paper-netaware/bab1}
\input{src/paper-netaware/bab2}
\input{src/paper-netaware/bab3}
\input{src/paper-netaware/bab4}
\input{src/paper-netaware/bab5}
...
\section{Network-Aware Global Pre-Eviction Filter Configuration (L1)}
\label{appendix:netaware_configL1}
\lstinputlisting[
    label={lst:netaware_configL1},
]{assets/codes/pseudos/netaware/configL1.psu}

\section{Network-Aware Strategy-Embedded Filter Configuration (L2)}
\label{appendix:netaware_configL2}
\lstinputlisting[
    label={lst:netaware_configL2},
